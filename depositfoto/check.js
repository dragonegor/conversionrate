    ...
    Backbone.$(function () {
        ...
        if ($('.icon-payment-googlepay').length) {
            ...
            function returnBlock() {
                ...
                var totalText = $('.order-info__price-cell:first-child').text().split(':')[0] + ':'
                if (width >= 1041) {
                    if ($('.billing-form__address').length == 0) {
                        $('.order-info__price-cell:first-child').text(totalText + ' ' + $('.order-info__price-full').text())
                    }

                    $('.billing-page__cell:first-child').append($('._process')).append($('.billing-settings.billing-settings_custom'));
                } else {
                    if ($('.billing-form__address').length == 0) {
                        $('.order-info__price-cell:first-child').text(totalText)
                    }

                    $('.billing-page__container').append($('.billing-settings.billing-settings_custom'));
                    $('.order-info.order-info_custom').append($('._process'))
                }
            }
            $('.billing-page__cell:first-child').append($('._process')).append($('.billing-settings.billing-settings_custom'));
            ...
            if ($('.checkout-trust-blocks').length === 0) {
                ...
                if ($('.billing-form__address').length != 0) {
                    $('.billing-page__cell:last').append(trust_blocks);
                } else {
                    $('.billing-page__wrap').after(trust_blocks)
                    // $('.order-info__price-cell:first-child').append(' ' + $('.order-info__price-full').text())
                ...
                $(".currency-select").on("click", function () {
                    var isCurrencyLoad
                    clearInterval(isCurrencyLoad);
                    isCurrencyLoad = setInterval(function () {
                        if ($('.cmp-currency-select-list').length) {
                            $('.cmp-currency-select-list__option._currency').on("click", function () {
                                location.reload();
                            })
                            clearInterval(isCurrencyLoad);
                        }
